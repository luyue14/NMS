$(document).ready(function() {
	...
	stage.add(scene);
	var widthOfCanvas = $(document).width()-325;
	...
	*/
	function afterRequestTopo(topoResult){
		...
		/* for(var i = 0; i < nodes.length; i++){
			if(nodes[i].entity.getLocation().x < 0){
				if(nodes[i].entity.getLocation().y < 0){
					nodes[i].entity.setLocation(0,0);
				}else if(nodes[i].entity.getLocation().y < heightOfCanvas){
					nodes[i].entity.setLocation(0,nodes[i].entity.getLocation().y);
				}else{
					nodes[i].entity.setLocation(0,heightOfCanvas-heightOfImage);
				}
			}else if(nodes[i].entity.getLocation().x < widthOfCanvas){
				if(nodes[i].entity.getLocation().y < 0){
					nodes[i].entity.setLocation(nodes[i].entity.getLocation().x,0);
				}else if(nodes[i].entity.getLocation().y < heightOfCanvas){
					nodes[i].entity.setLocation(nodes[i].entity.getLocation().x,nodes[i].entity.getLocation().y);
				}else{
					nodes[i].entity.setLocation(nodes[i].entity.getLocation().x,heightOfCanvas-heightOfImage);
				}
			} else{
				if(nodes[i].entity.getLocation().y < 0){
					nodes[i].entity.setLocation(widthOfCanvas-widthOfImage,0);
				}else if(nodes[i].entity.getLocation().y < heightOfCanvas){
					nodes[i].entity.setLocation(widthOfCanvas-widthOfImage,nodes[i].entity.getLocation().y);
				}else{
					nodes[i].entity.setLocation(widthOfCanvas-widthOfImage,heightOfCanvas-heightOfImage);
				}
			}
		} */
        wait(30000);
